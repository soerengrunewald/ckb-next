...
void KeyWidget::paintGL(){
    ...
    for(const Key& key : keyMap){
        ...
        if(((model != KeyMap::STRAFE && model != KeyMap::K95P && model != KeyMap::K100 && model != KeyMap::K70MK2 && model != KeyMap::STRAFE_MK2 && model != KeyMap::K70_TKL && model != KeyMap::K70_PRO && model != KeyMap::K70_PRO_RGB) && (!strcmp(key.name, "mr") || !strcmp(key.name, "m1") || !strcmp(key.name, "m2") || !strcmp(key.name, "m3")
            ...
            if(!strcmp(key.name, "zone7")){
                ...
                painter.drawRect(QRectF(x * drawInfoScale, y * drawInfoScale, w * drawInfoScale, h * drawInfoScale));
        } else if ((model == KeyMap::K70MK2 || model == KeyMap::STRAFE_MK2 || model == KeyMap::K70_TKL || model == KeyMap::K70_PRO || model == KeyMap::K70_PRO_RGB) && key.friendlyName().startsWith("Logo")) {
            w += 10.f;
            x -= 5.f;
            painter.drawRect(QRectF(x * drawInfoScale, y * drawInfoScale, w * drawInfoScale, h * drawInfoScale));
